...
# Basics of pygame
# making the game snake from Nokia 3310

pg.init()  # init. need to do this every script, kinda like tkinter's Tk()
...
### Apple ###
def rand_Apple():
    randAppleX = randrange(aSize, windowSize[0] - aSize)  # ,sSize)
    randAppleY = randrange(aSize, windowSize[1] - aSize)  # ,sSize)
...
def game_Loop():
    ...
    while True:
        ...
        ### Apple Boundary ###
        if (leadX > randAppleX) and (leadX < randAppleX + aSize) or \
                        (leadX + sSize > randAppleX) and (leadX + sSize < randAppleX + aSize):
            # checks if you are in the X boundary
            if (leadY > randAppleY) and (leadY < randAppleY + aSize) or \
                            (leadY + sSize > randAppleY) and (leadY + sSize < randAppleY + aSize):
                # checks Y boundary
        ...
        snake(sSize, sList, sHead)  # calls function to draw snake
